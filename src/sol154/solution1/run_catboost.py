...
# ── Конфигурация ──
from src.sol154.config import DATA_DIR, SOL154_SOLUTION1_CACHE as CACHE_DIR, SOL154_SOLUTION1_SUBMISSIONS as SUBMISSIONS
...
# Генерация фичей
def _period_frames(part_id, labels_lf):
...
def build_features_part(part_id, labels_lf, force=False):
    ...
    lf = _period_frames(part_id, labels_lf)
    TWO_PI = 2.0 * math.pi
...
def run():
    ...
    # Генерация фичей
    log("ГЕНЕРАЦИЯ ФИЧЕЙ")
    paths = [build_features_part(i, labels_lf, force=FORCE_REBUILD) for i in [1, 2, 3]]
