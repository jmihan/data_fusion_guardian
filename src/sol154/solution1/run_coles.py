...
import gc, time, warnings, functools
...
import logging
from src.sol154.config import SOL154_COLES_CACHE as CACHE_COLES, DATA_DIR
...
_fh = logging.FileHandler(CACHE_COLES / "coles.log", mode="w", encoding="utf-8")
...
class CustomerSeqDataset(Dataset):
    ...
    def _get_subsequence(self, idx, rng):
        ...
        for c in CAT_FEATURES:
            ...
            arr = np.clip(arr + 1, 0, CAT_CARDS[c] - 1)
            cat_data.append(arr)
...
class CoLESEncoder(nn.Module):
    ...
    def forward(self, cat, num, mask):
        ...
        pooled = (output * mask_exp).sum(dim=1) / mask_exp.sum(dim=1).clamp(min=1)
...
def extract_embeddings(model, sequences_df, batch_size=512):
    ...
    dataset = CustomerSeqDataset(sequences_df, seq_len=500)
    all_embeddings = []
    all_customer_ids = []
