...
def materialize_label_history_features(
    ...
    # float16 — stored as part of num_matrix, cast to float32 at collation time.
    out = np.zeros((len(customer_id), len(LABEL_HISTORY_NUM_COLS)), dtype=np.float16)
...
def load_and_preprocess(
    ...
    # float16 halves num_matrix RAM (191M x 33 x 2 = 12.6 GB vs 25.2 GB).
    # collate_segments already casts to float32 via torch.as_tensor(..., dtype=torch.float32).
    num_matrix = np.zeros((len(df), len(active_num_cols)), dtype=np.float16)
    base_num_col_index = {c: i for i, c in enumerate(BASE_NUM_COLS)}
    for c in BASE_NUM_COLS:
        num_matrix[:, base_num_col_index[c]] = df[c].to_numpy().astype(np.float16, copy=False)
    ...
    # Sanitize float16 overflow: values > 65504 become ±inf after float32→float16 cast,
    # which propagates as NaN through the model. Replace with 0.
    np.nan_to_num(num_matrix, nan=0.0, posinf=0.0, neginf=0.0, copy=False)
...
class MultiTaskTwoBranchMultiWindowModel(nn.Module):
    ...
    def _pool_from_pair_mask_mean(
        ...
        # Force bmm in float32 to avoid float16 overflow/div-by-zero under AMP
        weighted_sum = torch.bmm(weights, event_emb.float())
        weight_sum = weights.sum(dim=-1)

        hist = weighted_sum / weight_sum.clamp_min(1.0).unsqueeze(-1)
